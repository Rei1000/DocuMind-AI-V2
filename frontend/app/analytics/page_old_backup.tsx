...
export default function AnalyticsPage() {
  ...
  // NEU v2.7.0: States für neue Features
  const [mlModelInfo, setMlModelInfo] = useState<any>(null)
  const [cacheStats, setCacheStats] = useState<any>(null)
  const [backgroundStats, setBackgroundStats] = useState<any>(null)
  const [mlFeatureImportance, setMlFeatureImportance] = useState<any>(null)
  ...
  const loadMLModelInfo = async () => {
    try {
      const response = await fetch('/api/rag/ml/model-info', {
      ...
      console.error('Failed to load ML model info:', error)
    }
  }

  const loadCacheStats = async () => {
    try {
      const response = await fetch('/api/rag/analytics/shap/cache-stats', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const loadBackgroundStats = async () => {
    try {
      const response = await fetch('/api/rag/analytics/shap/background-stats', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const loadMLFeatureImportance = async () => {
    try {
      const response = await fetch('/api/rag/ml/feature-importance', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      })
      if (response.ok) {
        const data = await response.json()
        setMlFeatureImportance(data)
      }
    } catch (error) {
      console.error('Failed to load ML feature importance:', error)
    }
  }
  ...
  return (
    <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* NEU v2.7.0: Moderne Analytics-Komponenten */}
      <div className="mt-8 space-y-6">
        <div className="border-t border-gray-300 pt-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">
            🤖 ML & Performance Analytics (v2.7.0)
          </h2>
        </div>

        {/* ML Model Info & Stats Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Model Info */}
          {mlModelInfo && (
            <ModelInfoCard
              modelType={mlModelInfo.model_info?.model_type || mlModelInfo.model_type || 'none'}
              modelVersion={mlModelInfo.model_info?.model_version || mlModelInfo.model_version || 'none'}
              isReady={mlModelInfo.ready || false}
              featureNames={mlModelInfo.feature_names || mlModelInfo.model_info?.feature_names}
              trainingDataStats={mlModelInfo.training_data_stats}
            />
          )}

          {/* Cache Stats */}
          {cacheStats && cacheStats.cache_stats && (
            <CacheStatsCard
              cacheSize={cacheStats.cache_stats.cache_size}
              maxSize={cacheStats.cache_stats.max_size}
              hits={cacheStats.cache_stats.hits}
              misses={cacheStats.cache_stats.misses}
              hitRatePercent={cacheStats.cache_stats.hit_rate_percent}
              estimatedTimeSavedMinutes={cacheStats.performance_metrics?.estimated_time_saved_minutes || 0}
            />
          )}
          ...
          {backgroundStats && (
            <BackgroundStatsCard
              totalRecords={backgroundStats.total_records}
              backgroundDataShape={backgroundStats.background_data_shape}
              lastUpdate={backgroundStats.last_update}
              oldestRecord={backgroundStats.oldest_record}
              newestRecord={backgroundStats.newest_record}
            />
          )}
        </div>

        {/* ML Feature Importance */}
        {mlFeatureImportance && mlFeatureImportance.enabled && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              🎯 ML Feature Importance (Global)
            </h3>
            <p className="text-sm text-gray-600 mb-6">
              Wichtigkeit der Features im trainierten ML-Modell
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(mlFeatureImportance.feature_importance)
                .slice(0, 11)
